...
public class DeveloperTaskAllocator
{
    ...
    public String allocateTasks() {
        StringBuilder output = new StringBuilder();
        List<Resource> developers = loadDevelopers();
        ...
        for (Task task : tasks) {
            output.append("Task |  " + task.getName() + "\n");
            output.append("Based on Skill, Could be allocated to Developer(s)::\n");
            for (Resource dev : developers) {
                if (skillOnlyMatchStrategy.isMatch(dev, task)) {
                    output.append("- " + dev.getName() + "\n");
                }
            }
            output.append("Based on Expertise, Could be allocated to Developer(s)::\n");
            for (Resource dev : developers) {
